...
def get_list_of_resto_url(response):
    # my_urls = response.css('a.property_title::attr(href)').extract()
    return response.css('a.restaurants-list-ListCell__restaurantName--2aSdo ::attr(href)').extract()


def get_next_list_of_resto(response):
...
def get_list_of_review_url(reviews):
    return reviews.css('div.quote a ::attr(href)').extract()


def get_next_list_of_reviews(response):
    xpath = '//a[@class="nav next taLnk ui_button primary"]'
    # xpath = '//a[@class="nav next rndBtn ui_button primary"]'
    # next_page = response.css('div.js_pageLinks > a::attr(href)').extract()#[-1]
    # print('> RESTO NEXT PAGE')
    # if next_page == []:
    #     next_page = response.css('div.ratings_and_types').css('div.ui_pagination > a::attr(href)').extract()[0] 
    #     next_page_number = response.css('div.ratings_and_types').css('div.ui_pagination > a::attr(data-page-number)').extract()[-1]
    # else:       
    #     next_page = next_page[-1] 
    #     next_page_number = response.css('div.js_pageLinks > a::attr(data-page-number)').extract()[-1] 
...
def get_resto_url_from_review(response):
...
def get_reviewer_pseudo_from_review(review):
    try :
...
def get_reviewer_origin_from_review(review):
    try :
...
def get_reviewer_info_sup_from_review(review):
    try :
        reviewer_info_sup = review.css('span.badgetext::text').extract()
    except:
        reviewer_info_sup = None
    return reviewer_info_sup


def get_reviewer_url_from_review(reviewer_pseudo):
    return 'https://www.tripadvisor.co.uk/Profile/{}'.format(reviewer_pseudo)


def get_other_ratings_category_from_review(review):
    return
    try :
...
def get_other_ratings_value_from_review(review):
    try :
...
def get_nb_review_from_resto(general):
    return general.css('div.ratingContainer span.reviewCount::text').extract_first().split(' ')[0] 
...
def get_tel_number_from_resto(general):
    ...
    return tel_number
def get_url_menu_from_resto_list(general):
...
def get_description_from_resto(response):
    ...
    long_description = details.css('div.restaurants-details-card-DesktopView__desktopAboutText--1VvQH::text').extract_first()
