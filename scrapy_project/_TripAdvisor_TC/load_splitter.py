...
def cleaning_data(reviews):
    ...
    english_reviews = reviews[reviews.estimated_language == "en"]
...
reviews["estimated_language"] = reviews.review.apply(lambda x: "fr" if "é" in x else "en")


topics = ["service", "activities", "cost", "family", "food", "infrastructure"]
columns_to_save = ["review"]
name_of_group = ["groupe_" + str(i) for i in range(1,13)]

for topic in topics:
    reviews[topic] = None
    columns_to_save.append(topic)
    reviews[topic + "_sentiment"] = None
    columns_to_save.append(topic + "_sentiment")

reviews["group_name"] = ""
columns_to_save.append("group_name")
i=1
all_df = []
for name in name_of_group:
    reviews.loc[i*250:(i+1)*250, ["group_name"]] = name


    reviews = cleaning_data(reviews)

    to_save = reviews.loc[:3500, columns_to_save]

    i+=1



to_save.to_csv("full_group.csv", sep=';', encoding="utf-8")
